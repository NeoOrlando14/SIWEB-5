...
export async function GET(req) {
  try {
    ...
    // TOTAL ORDER (filtered by date range & status valid)
    // Status yang dihitung: selesai, diterima, Selesai (dari CSV upload)
    // Tidak termasuk: dibatalkan, Dibatalkan, proses, Proses
    const totalOrder = await prisma.transaksi.count({
      where: {
        status: {
          in: ["diterima", "selesai", "Selesai", "Diterima"]
        },
        tanggal: {
          gte: startDate,
          lte: endDate,
    ...
    // TOTAL SALES (filtered by date range & status valid)
    ...
    // PRODUK TERLARIS (filtered by date range & status valid)
    ...
    // GRAFIK DATA (dinamis berdasarkan filter, status valid only)
